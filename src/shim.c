...
read_frame(struct cc_shim *shim)
{
	...
	shim_debug("Read frame HeaderLength: %d, Version: %d, Payloadlen: %d,"
			"Type:%d, Opcode: %d, Err:%d\n",
...
handle_signals(struct cc_shim *shim) {
	...
	while (read(signal_pipe_fd[0], &sig, sizeof(sig)) != -1) {
		...
		if (! send_proxy_message(shim, frametype_command, cmd_signal, payload)) {
			shim_error("Could not send signal command to proxy %s\n",
					shim->proxy_address);
		}
